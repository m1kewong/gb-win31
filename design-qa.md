...
- States and affordances: the high-contrast 8 x 8 45-degree triangular pointer
  plus reversed caption communicates selection without the former extra marker.
...
3. Hardware-scale review later exposed a P2 cursor defect: the original arrow
   ended on its tile boundary, and an interim 8 x 16 version produced an
   awkward stem instead of a convincing cursor tail. The final pointer is a
   tail-free 45-degree triangle contained in one 8 x 8 tile, with a transparent
   right column and bottom row. Emulator smoke asserts its padding and bounds.
